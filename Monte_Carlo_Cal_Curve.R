...
vflask_w1 = c() # Vector containing values for the volumetric flask used to make the series dilution of solution 1 (mL)

vflask_s2 = c() # Vector containing values for the volumetric flask used to make the stock solution 2 (mL)

vflask_w2 = c() # Vector containing values for the volumetric flask used to make the series dilution of solution 2 (mL)
...
sd_vinj = 0.4 # Standard deviation of the injection process (µL)
...
sd_vflask = 0.01 # Standard deviation of the volumetric flask used CLASS A (mL)
...
for (i in seq_along(vflask_1)){
  ...
  check[i] = (k*m_1[i]*vpip_1[i]*vinj_1[i]*(m_2[i]/vflask_w2[i])*vflask_w1[i]*vflask_s1[i])/(vflask_s2[i]*vflask_w2[i]*k*m_2[i]*vpip_2[i]*vinj_2[i]*(m_1[i]/vflask_w1[i]))
}
...
hist(check,                                                 
     col = "blue",
     border = "black",
     main = "Figure A - Distribution of Accu%",
     xlab = "Accu% Standard Values",
     ylab = "Frequency")
